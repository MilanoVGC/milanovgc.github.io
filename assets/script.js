...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log("DEBUG: searchInput element:", searchInput); // <<<--- Check if element is found
    ...
    function extractData(xmlDoc) {
        ...
        try {
            ...
            if (tournamentData && tournamentInfoElement) { const city = tournamentData.querySelector('city')?.textContent; const country = tournamentData.querySelector('country')?.textContent; tournamentInfoElement.textContent = `${city ? city + ', ' : ''}${country || ''}`.trim(); }
            ...
            playerElements.forEach((player, index) => {
                ...
                if (birthdateText) { try { const yearMatch = birthdateText.match(/(\d{4})$/); if (yearMatch && yearMatch[1]) { birthYear = parseInt(yearMatch[1], 10); } else { console.warn(`extractData:  -> Could not parse year from birthdate="${birthdateText}" for player ${userId}. Expected MM/DD/YYYY.`); } } catch (e) { console.warn(`extractData:  -> Error parsing birthdate="${birthdateText}" for player ${userId}`, e); } } else { console.warn(`extractData:  -> Missing '<birthdate>' tag or content for player ${userId}`); }
                if (userId) { tempPlayersData[userId] = { id: userId, firstName, lastName, name: `${firstName} ${lastName}`.trim(), birthYear: birthYear }; } else { console.warn(`extractData: Player at index ${index} missing userid attribute.`); }
            });
    ...
    function getPlayerSwissWinPercentage(playerId, minPercentage) { const record = (standingsCache[playerId]?.record) ? standingsCache[playerId].record : calculatePlayerSwissRecord(playerId); const totalMatches = record.matchesPlayed; if (totalMatches === 0) { return minPercentage; } const winRate = record.wins / totalMatches; const finalWP = Math.max(winRate, minPercentage); return finalWP; }
    function calculateOWP(playerId, minPercentage) { const opponents = getSwissOpponents(playerId); if (opponents.length === 0) return 0; let totalWeightedOpponentWinPercentage = 0; let validOpponentCount = 0; opponents.forEach(oppId => { try { const opponentWinPerc = getPlayerSwissWinPercentage(oppId, minPercentage); const opponentDivision = getPlayerDivision(oppId); let weight = MASTERS_WEIGHT; if (opponentDivision === 'Junior' || opponentDivision === 'Senior') { weight = JUNIOR_SENIOR_WEIGHT; } if (typeof opponentWinPerc === 'number' && !isNaN(opponentWinPerc)) { totalWeightedOpponentWinPercentage += (opponentWinPerc * weight); validOpponentCount++; } else { console.warn(`>>> calculateOWP (${playerId}): Invalid Win% (${opponentWinPerc}) calculated for opponent ${oppId}. Skipping opponent.`); } } catch (opponentError) { console.error(`>>> calculateOWP (${playerId}): Error processing opponent ${oppId}:`, opponentError); } }); const result = (validOpponentCount > 0) ? (totalWeightedOpponentWinPercentage / validOpponentCount) : 0; return result; }
    ...
    function updateUI() {
        ...
        try {
            if (Object.keys(playersData).length === 0 || roundsData.length === 0) { console.log("updateUI: No player or round data available for pairings."); if (lastKnownTimeElapsed === -1) { if(loadingMessage){ loadingMessage.textContent = "Waiting for tournament data..."; loadingMessage.style.display = 'block';} } else { if(loadingMessage){ loadingMessage.textContent = "No player/round data found. Check console for errors."; loadingMessage.style.display = 'block'; loadingMessage.style.color = '#dc3545'; } } if(pairingsTable) pairingsTable.style.display = 'none'; if(currentRoundTitle) currentRoundTitle.style.display = 'none'; if(roundTabsContainer) roundTabsContainer.innerHTML = ''; if (standingsContainer) standingsContainer.style.display = 'none'; return; }
            ...
            displayRound(currentRound); updateActiveTab(); if (loadingMessage) loadingMessage.style.display = 'none'; if (pairingsTable) pairingsTable.style.display = 'table'; if (currentRoundTitle) currentRoundTitle.style.display = 'block';
             // Ensure filterTable is called only if searchInput exists
             if (searchInput) {
                 filterTable();
             } else {
        ...
        if (Object.keys(playersData).length > 0 && roundsData.length > 0) {
            try {
                console.log("updateUI: >>> Starting Standings Update Logic <<<"); const swissRoundsExist = roundsData.some(r => r.type === "3"); console.log(`updateUI: Swiss rounds exist? ${swissRoundsExist}`);
                if (swissRoundsExist) { const standingsData = calculateSwissStandings(); console.log(`updateUI: Calculated standings data length: ${standingsData ? standingsData.length : 'null'}`); const sortedStandings = sortStandings(standingsData); console.log(`updateUI: Sorted standings data length: ${sortedStandings ? sortedStandings.length : 'null'}`); displayStandings(sortedStandings); console.log("updateUI: Called displayStandings."); }
                else { console.log("updateUI: No swiss rounds exist, ensuring standings are hidden."); if (standingsContainer) standingsContainer.style.display = 'none'; if (noStandingsMsg) noStandingsMsg.style.display = 'block'; if (standingsLoadingMsg) standingsLoadingMsg.style.display = 'none'; }
                console.log("updateUI: >>> Standings section processing complete <<<");
            } catch (error) {
                 console.error("updateUI: CRITICAL Error during standings update block:", error); if (standingsLoadingMsg) standingsLoadingMsg.style.display = 'none'; if (noStandingsMsg) { noStandingsMsg.style.display = 'block'; noStandingsMsg.textContent = `Error updating standings: ${error.message}. Check console.`; } if (standingsContainer) standingsContainer.style.display = 'block'; if (standingsTableBody) standingsTableBody.innerHTML = '';
            }
        } else { console.log("updateUI: Skipping standings update due to lack of player/round data."); if (standingsContainer) standingsContainer.style.display = 'none'; }
    ...
    // --- Event Listeners ---
    if(searchInput) { // Add check here too
